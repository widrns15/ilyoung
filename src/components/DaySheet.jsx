...
export default function DaySheet({ day, events, txs, profiles, onClose, onAddEvent, onAddTx, onEditEvent, onEditTx, onReorderEvents }) {
  ...
  useEffect(() => { setItems(dayEvents) }, [eventsSig]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const rowMove = (e) => {
    ...
    if (!p) return
    if (!p.dragging) {
      if (Math.abs(e.clientY - p.y) > 8) { clearTimeout(p.timer); press.current = null }
      return
    }
    const rows = listRef.current ? [...listRef.current.querySelectorAll('[data-row]')] : []
        ...
        <div className="card" ref={listRef}>
          ...
          {items.map((ev) => (
            ...
            >
              ...
              {canReorder && !ev.virtual ? <span className="grip" aria-hidden>⠿</span> : <span className="t2">›</span>}
            </button>
          ))}
          {items.length > 0 && (
